...
SalesReturnsWidget::SalesReturnsWidget(QWidget *parent) : QWidget(parent) {
  auto *layout = new QVBoxLayout(this);
  layout->setSpacing(12);
  setLayoutDirection(Qt::RightToLeft);

  auto *title = new QLabel("🔄  مرتجعات المبيعات");
  title->setStyleSheet(
  ...
  m_table->setHorizontalHeaderLabels({"#", "رقم المرتجع", "التاريخ", "العميل",
  ...
  connect(filterBtn, &QPushButton::clicked, this,
          &SalesReturnsWidget::loadData);
  connect(addBtn, &QPushButton::clicked, this, &SalesReturnsWidget::onAdd);
  loadData();
}

void SalesReturnsWidget::refresh() { loadData(); }

void SalesReturnsWidget::loadData() {
  m_table->setRowCount(0);
  auto q = DatabaseManager::instance().getSalesReturns(
...
void SalesReturnsWidget::onAdd() {
  QDialog dlg(this);
  dlg.setWindowTitle("إضافة مرتجع مبيعات");
  ...
  form->addRow("رقم فاتورة البيع:", invSpin);
  ...
  connect(saveBtn, &QPushButton::clicked, [&]() {
    DatabaseManager::instance().createSalesReturn(
        invSpin->value(), custCombo->currentData().toInt(),
